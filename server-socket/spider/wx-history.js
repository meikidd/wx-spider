...
module.exports = async function(socket, accountId) {
  ...
  try {
    ...
    while(waitForLastPage) {
      let response = await getHistoryArticle({__biz, uin, key, pass_ticket, frommsgid}, cookies);
...
async function getHistoryArticle(params, cookies) {
